...
public class Generator : MonoBehaviour
{
    ...
    [SerializeField] private SavedDungeonLayout preloadedLayout;
    ...
    /// <summary>
    /// Captures the current dungeon's structural layout (room positions, sizes, and corridor tiles)
    /// into a SavedDungeonLayout ScriptableObject instance. Returns null if no dungeon exists.
    /// </summary>
    public SavedDungeonLayout CreateLayoutSnapshot()
    {
        if (parentRooms == null || parentRooms.Length == 0 || area == null)
        {
            Debug.LogError("No dungeon has been generated. Generate a dungeon first before saving.");
            return null;
        }

        SavedDungeonLayout layout = ScriptableObject.CreateInstance<SavedDungeonLayout>();
        layout.areaWidth = areaWidth;
        layout.areaHeight = areaHeight;

        // Save room positions, sizes, and boss room flag
        for (int i = 0; i < parentRooms.Length; i++)
        {
            RoomLayoutData roomData = new RoomLayoutData();
            roomData.position = roomPos[i];
            roomData.size = roomArea[i];
            roomData.isBossRoom = (parentRooms[i] == bossSpawnRoom);
            layout.rooms.Add(roomData);
        }

        // Save corridor tiles (floor tiles that are NOT within any room's bounding rectangle)
        for (int x = 0; x < areaWidth; x++)
        {
            for (int y = 0; y < areaHeight; y++)
            {
                if (area[x, y] != null && area[x, y].CompareTag("Floor"))
                {
                    bool isInRoom = false;
                    for (int i = 0; i < parentRooms.Length; i++)
                    {
                        if (x >= roomPos[i].x && x < roomPos[i].x + roomArea[i].x &&
                            y >= roomPos[i].y && y < roomPos[i].y + roomArea[i].y)
                        {
                            isInRoom = true;
                            break;
                        }
                    ...
                    if (!isInRoom)
                    {
                        layout.corridorTiles.Add(new Vector2Int(x, y));
                    }
                }
            }
        }

        layout.savedObjects.Clear();
        DungeonItem[] items = GetComponentsInChildren<DungeonItem>(true);
        foreach (DungeonItem item in items)
        {
            SavedObjectData objData = new SavedObjectData();
            objData.prefabName = item.prefabName;
            objData.tag = item.gameObject.tag;
            objData.position = item.transform.position;
            objData.rotation = item.transform.rotation;
            objData.scale = item.transform.localScale;
            objData.parentName = item.transform.parent != null ? item.transform.parent.name : "";
            layout.savedObjects.Add(objData);
        }

        Debug.Log("Layout snapshot created: " + layout.rooms.Count + " rooms, " + layout.corridorTiles.Count + " corridor tiles, " + layout.savedObjects.Count + " saved objects.");
        return layout;
    }

    /// <summary>
    /// Rebuilds the dungeon from a saved layout asset. Restores the floor grid from saved
    /// room/corridor coordinates, then runs the full visual pipeline (walls, corners, doors,
    /// content, entities) to procedurally dress the restored structure.
    /// </summary>
    private void LoadFromSavedLayout()
    {
        if (generationSettings == null)
        {
            Debug.LogError("Need Generation Settings to load a saved dungeon layout.");
            return;
        }

        if (preloadedLayout.rooms.Count == 0)
        {
            Debug.LogError("Saved layout contains no rooms.");
            return;
        }
        ...
        // Override grid dimensions and room count from saved layout
        areaWidth = preloadedLayout.areaWidth;
        areaHeight = preloadedLayout.areaHeight;
        rooms = preloadedLayout.rooms.Count;
        ...
        // Reset corridor state for clean rebuild
        corridors.Clear();
        corridorNumber = 0;
        numberOfRooms = 0;
        ...
        // === PHASE 1: Recreate rooms from saved data ===
        int bossRoomIndex = -1;
        for (int i = 0; i < rooms; i++)
        {
            RoomLayoutData savedRoom = preloadedLayout.rooms[i];
            roomPos[i] = savedRoom.position;
            roomArea[i] = savedRoom.size;

            if (savedRoom.isBossRoom)
                bossRoomIndex = i;

            // Create room parent at the calculated center point
            numberOfRooms++;
            Vector2 centerPoint = CalculateCenterPoint(roomArea[i], roomPos[i]);
            parents[i] = Instantiate(emptyParentRoom, new Vector3(centerPoint.x, 0, centerPoint.y), Quaternion.identity, gameObject.transform);
            ...
            // Instantiate floor tiles in the same iteration order as CreateRooms (x then y)
            // This ensures child ordering matches what GenerateRoomWallObjs expects for corner detection
            for (int a = 0; a < roomArea[i].x; a++)
            {
                for (int b = 0; b < roomArea[i].y; b++)
                {
                    int tileX = roomPos[i].x + a;
                    int tileY = roomPos[i].y + b;

                    int randomFlooring = Random.Range(0, flooring.Count);
                    area[tileX, tileY] = Instantiate(flooring[randomFlooring].gameObject, new Vector3(tileX, 0, tileY), Quaternion.identity, parents[i].transform);
                    area[tileX, tileY].name = "Floor" + " (" + tileX + ", " + tileY + ")";
                }
            }
        ...
        // === PHASE 2: Calculate player spawn room (furthest from boss) ===
        float maxDist = 0f;
        playerSpawnRoom = parentRooms[0];
        for (int i = 0; i < rooms; i++)
        {
            if (bossRoomIndex >= 0 && i != bossRoomIndex)
            {
                float dist = Vector3.Distance(parents[bossRoomIndex].transform.position, parents[i].transform.position);
                if (dist > maxDist)
                {
                    maxDist = dist;
                    playerSpawnRoom = parentRooms[i];
                }
            }
        }

        // === PHASE 3: Recreate corridor tiles grouped into connected components ===
        if (preloadedLayout.corridorTiles.Count > 0)
        {
            HashSet<Vector2Int> remaining = new HashSet<Vector2Int>(preloadedLayout.corridorTiles);

            while (remaining.Count > 0)
            {
                // Get a starting tile
                Vector2Int start = default;
                foreach (Vector2Int v in remaining)
                {
                    start = v;
                    break;
                }
                ...
                // BFS to collect all connected corridor tiles in this component
                ...
                // Create a corridor parent for this connected group
                GameObject corridorParent = Instantiate(emptyObject, gameObject.transform);
                corridorParent.name = "Corridor " + corridors.Count;
                corridors.Add(corridorParent);

                // Instantiate floor tiles for this corridor group
                foreach (Vector2Int tile in component)
                {
                    int randomFloor = Random.Range(0, flooring.Count);
                    GameObject corridorFloor = Instantiate(flooring[randomFloor].gameObject, new Vector3(tile.x, 0, tile.y), Quaternion.identity, corridorParent.transform);
                    corridorFloor.name = "Floor" + " (" + tile.x + ", " + tile.y + ")";
                    area[tile.x, tile.y] = corridorFloor;
                }
            }
        }

        // === PHASE 4: Run the full visual generation pipeline on the restored grid ===
        GenerateWalls();
        ...
        if (preloadedLayout.savedObjects != null && preloadedLayout.savedObjects.Count > 0)
        {
            RestoreSavedObjects();
        }
        else
        ...
        Debug.Log("Dungeon loaded from saved layout: " + rooms + " rooms.");
    }
    ...
    private void RestoreSavedObjects()
    {
        ...
        if (bossSpawnRoom != null) bossSpawnRoom.name = "Boss Room";

        Transform[] allTransforms = GetComponentsInChildren<Transform>(true);

        foreach (SavedObjectData savedObj in preloadedLayout.savedObjects)
        {
            ObjectList prefabList = FindPrefabByName(savedObj.prefabName);
            if (prefabList != null && prefabList.gameObject != null)
            {
                Transform parent = transform;
                if (!string.IsNullOrEmpty(savedObj.parentName))
                {
                    foreach (Transform t in allTransforms)
                    {
                        if (t.name == savedObj.parentName)
                        {
                            parent = t;
                            break;
                        }
                ...
                GameObject obj = Instantiate(prefabList.gameObject, savedObj.position, savedObj.rotation, parent);
                obj.name = savedObj.prefabName;
                obj.transform.localScale = savedObj.scale;
                obj.tag = savedObj.tag;
                
                DungeonItem item = obj.AddComponent<DungeonItem>();
                item.prefabName = savedObj.prefabName;
            }
        }
    }

    private ObjectList FindPrefabByName(string name)
    {
        if (bosses != null) { foreach (var obj in bosses) if (obj.name == name) return obj; }
        if (enemies != null) { foreach (var obj in enemies) if (obj.name == name) return obj; }
        if (breakables != null) { foreach (var obj in breakables) if (obj.name == name) return obj; }
        if (staticObjects != null) { foreach (var obj in staticObjects) if (obj.name == name) return obj; }
        if (wallObjects != null) { foreach (var obj in wallObjects) if (obj.name == name) return obj; }
        if (middleObjects != null) { foreach (var obj in middleObjects) if (obj.name == name) return obj; }
        return null;
    }
    ...
    private GameObject InstantiateContent(ObjectList content, Vector3 position, Quaternion rotation, Transform parent)
    {
        position.y += content.yOffset;
        Quaternion customRotation = rotation * Quaternion.Euler(content.rotationOffset);
        GameObject obj = Instantiate(content.gameObject, position, customRotation, parent);
        obj.name = content.name;
        DungeonItem item = obj.AddComponent<DungeonItem>();
        item.prefabName = content.name;
        return obj;
    }
    ...
    private void SpawnEntities()
    {
        ...
        if(bosses.Count > 0)
        {
            ...
            boss = InstantiateContent(bosses[randomBoss], bossSpawnRoom.transform.position, Quaternion.identity, bossSpawnRoom.transform);
        ...
        if(wallObjects.Count > 0)
        {
            ...
            for (int j = 0; j < parentRooms[i].Corners.Count; j++)
            {
                ...
                if (tempVec3.x == 1)
                {
                    ...
                    GameObject tempWallObject = InstantiateContent(wallObjects[randomWallObject], new Vector3(area[tempVec3.y, tempVec3.z].transform.position.x + tempTorchDisplacement.x, tempTorchDisplacement.y, area[tempVec3.y, tempVec3.z].transform.position.z + tempTorchDisplacement.z), Quaternion.Euler(0, extraYAngle, 0), parentRooms[i].transform);
                }
                else
                {
                    //Remove from list
                ...
                if( corridors[i].transform.childCount > wallObjConsequitiveDistance)
                {
                    ...
                    for (int j = 0; j < corridors[i].transform.childCount; j++)
                    {
                        if(j > 0 && corridors[i].transform.GetChild(j).CompareTag("Floor"))
                        {
                            ...
                            {
                                int randomWallObject = Random.Range(0, wallObjects.Count);
                                GameObject tempWallObject = InstantiateContent(wallObjects[randomWallObject], new Vector3(position.x + wallObjDisplacements[randomWallObject].z, wallObjDisplacements[randomWallObject].y, position.z + wallObjDisplacements[randomWallObject].x), Quaternion.Euler(0, -90, 0), corridors[i].transform);
                                checkIteration = 0;
                            }
                            ...
                            {
                                int randomWallObject = Random.Range(0, wallObjects.Count);
                                GameObject tempWallObject = InstantiateContent(wallObjects[randomWallObject], new Vector3(position.x + wallObjDisplacements[randomWallObject].x, wallObjDisplacements[randomWallObject].y, position.z + wallObjDisplacements[randomWallObject].z), Quaternion.Euler(0, 180, 0), corridors[i].transform);
                                checkIteration = 0;  
                            }
                    ...
                    for (int a = 0; a < enem; a++)
                    {
                        ...
                        do
                        {
                            ...
                            if(tempChild.CompareTag("Floor") && !parentRooms[i].MidPoints.Contains(tempChild) && !parentRooms[i].Corners.Contains(tempChild))
                            {
                                ...
                                GameObject tempEnemy = InstantiateContent(Enemies[randomEnemy], new Vector3(pos.x, pos.y + Enemies[randomEnemy].gameObject.transform.position.y, pos.z), Quaternion.Euler(0, randomRotation, 0), parentRooms[i].transform);
            ...
            for (int a = 0; a < parentRooms.Length; a++)
            {
                for (int b = 0; b < parentRooms[a].MidPoints.Count; b++)
                {
                    ...
                    if(randomChange > 0 && randomChange < middleObjectsRate)
                    {
                        ...
                        {
                            GameObject tempMiddleObject = InstantiateContent(middleObjects[randomMiddleObject], parentRooms[a].MidPoints[b].transform.position, Quaternion.Euler(0, randomRotation, 0), parentRooms[a].transform);
                        }
                    }
                }
            }
    ...
    //TODO: Make functions to compact code
    private void RandomiseContent(ref int randomNumber, ref int staticOrBreakable, ref ObjectList tempContent, ref Vector2Int coords, ref int i, ref int j, ref Vector2Int additive)
    {
        ...
        if(!CheckIfNull(area[coords.x + additive.x, coords.y + additive.y], parentRooms[i].Corners[j]) && area[coords.x + additive.x, coords.y + additive.y].transform.parent != null)
        {
            if (randomNumber > 0 && randomNumber <= cornerObjectsRate)
            {
                ...
                {
                    area[coords.x + additive.x, coords.y + additive.y] = InstantiateContent(tempContent, new Vector3(area[coords.x + additive.x, coords.y + additive.y].transform.position.x, tempContent.gameObject.transform.position.y, area[coords.x + additive.x, coords.y + additive.y].transform.position.z), Quaternion.identity, parentRooms[i].Corners[j].transform.parent.transform);
                }
            }
        }
    }
    ...
    //TODO: Make functions to compact code
    private void CreateDestroyableRows(ref int staticOrBreakable, ref ObjectList tempContent, ref Vector2Int coords, ref int i, ref int j, ref Vector2Int additive)
    {
